...
CUSTOM_ALIVE_TEXT = Config.CUSTOM_ALIVE_TEXT or "☠︎︎༒︎✞︎ DarkCrews Leader Bot Ready To Fuck ✞︎༒︎☠︎︎"
...
async def amireallyalive(alive):
    ...
    if CAT_IMG:
        ...
        cat_caption += (
            f"<b>{EMOJI} DarkCrew Version :</b> <code>{catversion}</code>\n"
        )
...
async def amireallyalive(alive):
    ...
    reply_to_id = await reply_id(alive)
    cat_caption = f"**DarkCrews Leader Bot Ready To Fuck**\n"
    cat_caption += f"**  -Owner :** {mention}\n"
    cat_caption += f"**  -Python Version :** `{python_version()}\n`"
    cat_caption += f"**  -Telethon version :** `{version.__version__}\n`"
    cat_caption += f"**  -Darkcrew Version :** `{catversion}`\n"
